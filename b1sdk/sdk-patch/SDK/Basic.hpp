...
//
// Generated by Dumper-7, then modified.
//
// Changes: GObjects/AppendString/GNames/ProcessEvent are `inline` again so
...
#ifdef DUMPER7_NO_ASSERTS
#include "../AssertionsStub.inl"
#else
...
{
	/* ---------------------------------------------------------------------
	 * PATCHED for wukong-minimap.
	 *
	 * Dumper-7 emits these as `constexpr`, which hard-codes the SDK to one
	 * exact build of b1-Win64-Shipping.exe. They are `inline` here so that
	 * b1Init() can overwrite them with values found by signature scanning
	 * at runtime -- that survives most game patches.
	 *
	 ...
	 * makes UWorld::GetWorld() fall back to GEngine->GameViewport->World,
	 * which is found by walking GObjects and therefore needs no offset.
	 * dumped: 0x1D5E3770 */
	constexpr int32 GWorld            = 0x0;

	/* vtable index of UObject::ProcessEvent. Still version dependent -- if
	 * the engine gains or loses a virtual this must be re-dumped. */
